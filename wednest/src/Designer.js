...
const Designer = () => {
  const [designerVendors, setDesignerVendors] = useState([]);

  useEffect(() => {
    fetch("http://localhost:3000/api/vendors/type/Designer")
      .then((response) => response.json())
      .then((data) => {
        if (data.status === "success") {
          setDesignerVendors(data.data);
        } else {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold text-center mb-4">Designer Vendors</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {designerVendors.length > 0 ? (
          designerVendors.map((vendor) => (
            <div
              key={vendor._id}
